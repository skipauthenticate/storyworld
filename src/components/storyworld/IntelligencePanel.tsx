    ...
    <div className="flex flex-col items-center text-center py-3 px-3 space-y-3">
      {phase === "idle" && !hasEnrichment && queueState?.status !== "paused" && (
        <div className="flex flex-col items-center gap-2 py-2">
          ...
          <span className="text-[11px] text-muted-foreground">No enrichment yet</span>
          <button
            onClick={handleEnrich}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 text-primary rounded-lg text-[11px] hover:bg-primary/20 transition-colors"
          >
            <Zap className="w-3 h-3" />
            Start Enrichment
          </button>
          <span className="text-[9px] text-muted-foreground/60">AI analysis of characters, themes &amp; passages</span>
        </div>
      )}

      {phase === "idle" && queueState?.status === "paused" && (
        <button
          onClick={handleEnrich}
      ...
      {/* Minimal processing indicator — main progress shown in header */}
      {isProcessing && (
        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <Loader2 className="w-3 h-3 text-primary animate-spin" />
            {PHASE_LABELS[phase]}
            {isDownloading && <span className="text-[9px]">({llmProgress}%)</span>}
          </span>
          <button
            onClick={cancel}
